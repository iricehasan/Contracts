...
use cosmwasm_std::{
    to_json_binary, Binary, CosmosMsg, Deps, DepsMut, Env, Event, MessageInfo, Response, StdResult,
    Timestamp, WasmMsg,
};
...
use crate::msg::{
    AdminAddressResponse, ExecuteMsg, InstantiateMsg, QueryMsg, StakingsResponse,
    WhitelistedNftAddressesResponse,
};
use crate::state::{Config, Staking, CONFIG, STAKINGS};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::RemoveCollection { nft_addr } => {
            execute_remove_collection(deps, env, info, nft_addr)
        }
...
pub fn execute_unstake(
    ...
    let mut stakings_state = STAKINGS
        .may_load(deps.storage, info.sender.to_string().clone())?
        .unwrap();
...
pub fn execute_claim(
    ...
    if staking_info.end_timestamp > env.block.time {
        return Err(ContractError::StillUnbounding {
            will_finish: staking_info.end_timestamp,
        });
    }
...
pub fn execute_add_collection(
    ...
    Ok(Response::new()
        .add_event(Event::new("Collection added").add_attribute("nft_addr", nft_addr.clone())))
}
...
pub fn execute_remove_collection(
    ...
    if let Some(index) = config
        .whitelisted_nft_addrs
        .iter()
        .position(|nft_address| *nft_address == nft_addr)
    {
    ...
    Ok(Response::new()
        .add_event(Event::new("Collection removed").add_attribute("nft_addr", nft_addr.clone())))
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::WhitelistedNftAddresses {} => {
            to_json_binary(&query_whitelisted_nft_addresses(deps)?)
        }
        QueryMsg::StakingsByAddress { address } => {
            to_json_binary(&query_stakings_by_address(deps, address)?)
        }
    }
}
...
pub fn query_stakings_by_address(deps: Deps, address: String) -> StdResult<StakingsResponse> {
    ...
    Ok(StakingsResponse { stakings: stakings })
}
...
pub fn query_whitelisted_nft_addresses(deps: Deps) -> StdResult<WhitelistedNftAddressesResponse> {
    ...
    let nft_addrs_string: Vec<String> = nft_addrs
        .into_iter()
